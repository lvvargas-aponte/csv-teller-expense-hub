"""One-shot backfill: rewrite transaction institutions to canonical names.

Usage:
    # Dry run — print the diff, write nothing:
    docker compose run --rm backend python -m scripts.normalize_institutions --check

    # Live run — apply the mapping in-place:
    docker compose run --rm backend python -m scripts.normalize_institutions

The mapping lives in ``backend/institution_normalizer.py`` (single source of
truth shared with live ingest paths, which normalize in
``csv_parser.Transaction.__post_init__``). Rows written before that landed can
carry either spelling — ``Chase`` and ``Chase Bank`` — which renders as two
institutions in the transactions table. This script is idempotent: a second run
after a successful first finds zero rows to change.
...
mutating the ``institution`` field.
"""
...
from institution_normalizer import normalize  # noqa: E402
...
def _print_diff(changes: list[tuple[str, str, str]], delta: Counter, total: int) -> None:
    ...
    if not changes:
        print("Nothing to do — institutions are already canonical.")
        return
