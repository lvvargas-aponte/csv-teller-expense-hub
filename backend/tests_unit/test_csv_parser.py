...
class TestBarclaysParser:
    ...
    def test_category_stays_none_not_direction_string(self):
        """Barclays' 'Category' column is DEBIT/CREDIT (direction), not a real
        merchant category. Parser should leave `category` None and encode the
        direction in `transaction_type` instead."""
        txns = BarclaysParser().parse(BARCLAYS_VALID)
        assert all(t.category is None for t in txns)
        assert all(t.transaction_type in ("debit", "credit") for t in txns)

    def test_direction_column_drives_transaction_type(self):
        """Regression: a DEBIT row arrives with a negative amount. The amount
        sign is Barclays' ledger convention, not the transaction direction —
        relying on it would flip every purchase to 'credit'."""
        csv = (
            "Transaction Date,Description,Category,Amount\n"
            "01/15/2024,SQ TATACO,DEBIT,-113.45\n"            # purchase
            "01/16/2024,Payment Received,CREDIT,29.00\n"      # payment in
            "01/17/2024,REFUND,CREDIT,-50.00\n"               # rare: negative credit
        )
        txns = BarclaysParser().parse(csv)
        assert txns[0].transaction_type == "debit"   # was "credit" before fix
